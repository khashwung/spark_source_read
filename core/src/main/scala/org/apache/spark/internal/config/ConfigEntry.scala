...
private[spark] abstract class ConfigEntry[T] (
  ...
  // stringConverter是一个Funciton1类型，也可以将函数类型作为类的成员
  ...
  // 核心方法，调用reader获取Option[String]，不是String！！
...
// 上面的ConfigEntry是一个抽象类，定义若干方法并实现了若干方法，下面是它的若干个实现：
// ConfigEntryWithDefault和ConfigEntryWithDefaultFunction
...
private[spark] object ConfigEntry {
  ...
  // key是entry的key，而值是entry本身
  private val knownConfigs = new java.util.concurrent.ConcurrentHashMap[String, ConfigEntry[_]]()

  def registerEntry(entry: ConfigEntry[_]): Unit = {
    // HashMap以及ConcurrentHashMap等都有一个putIfAbsent方法，非常实用！！
    // 返回之前key对应value（如果之前有该key），返回null（如果之前没有该key）
    val existing = knownConfigs.putIfAbsent(entry.key, entry)
    // Predef里的一个判定参数是否满足条件的断言
