 ...
 * flush an immutable view of the entity to the app state store.
  * 每个live的实例都是用来表示某个entity的变化状态，并且周期性地生成一个view到app state store中去
  * LiveEntity是父类，其它具体的实体继承该父类
 */
private[spark] abstract class LiveEntity {

  // 最后一次写status store的时间，一开始为-1，后面为上一次写的时间
   ...
   * latest information collected by the listener.
    * 返回实体数据的新的view，然后将该view存进status store中去
   */
...
private class LiveTask(
  ...
  // 先创建一个默认的TaskMetrics，里面含有TaskMetrics的api，默认所有的元素都为-1
   ...
   * values.
    * 注意，该函数返回的是difference，即TaskMetrics的Delta！！
   */
  def updateMetrics(metrics: TaskMetrics): v1.TaskMetrics = {
    if (metrics != null) {
      ...
      if (old.executorDeserializeTime >= 0L) {
        // 如果以前有执行过，那么就返回delta，如果以前没有老的metrics，那么就返回最新的metrics
