...
private[spark] class AppStatusListener(
  ...
  // 所有的live entity，这些都是要通过rest传到ui前端的信息
  ...
  // 达到允许的最大执行器数时，就会触发trigger，清除所有的count。以下同理
  ...
  /**
    * Listener里面总是有很多onEvent事件！！下面不同的事件都会传入不同的事件类型!!
    * 然后就根据这些事件类型更新所有的HashMap存储的那些liveEntity！！
    *
     */
  ...
  override def onApplicationStart(event: SparkListenerApplicationStart): Unit = {
    ...
    event.driverLogs.foreach { logs =>
      // 根据键值获取LiveExecutors对象，这是LiveEntity.scala里定义的
  ...
  override def onEnvironmentUpdate(event: SparkListenerEnvironmentUpdate): Unit = {
    // 每个event都带有这个event特有的一些信息
    val details = event.environmentDetails

    // _*表示任意类型的变长类型，这里将变长的Seq[(String,String)]转成Map
    val jvmInfo = Map(details("JVM Information"): _*)
    val runtime = new v1.RuntimeInfo(
      jvmInfo.get("Java Version").orNull,  // Option类型的orNull方法
  ...
  private def flush(): Unit = {
    val now = System.nanoTime()
    // liveStages和下面所有的livexx都是HashMap，values取所有的元素
    // 将这些集合里的所有元素都更新到kvstore里去
  ...
  private def getOrCreateExecutor(executorId: String, addTime: Long): LiveExecutor = {
    // HashMap的getOrElseUpdate方法，这是scala中mutable.HashMap里的方法，如果该键存在，返回，没有更新
    // 这是线程不安全的方法，因为传入的第二个参数是=>op，多个线程的副作用会计算多次
  ...
  private def cleanupExecutors(count: Long): Unit = {
    ...
    // 因为activeExecutorCount是volatile类型，因此是线程级可见的
    val dead = count - activeExecutorCount

    if (dead > threshold) {
      // 计算需要清除多少个执行器
      val countToDelete = calculateNumberToRemove(dead, threshold)
      // index:根据active的index进行迭代
      // max: 迭代到max数的时候就停止迭代
      // first: 从第一个值为false的地方开始迭代
      // last: 迭代到最后一个值为false的位置
