 ...
 * execution. To access the TaskContext for a running task, use:
  * 在执行Task的时候，会有这样的一个Task上下文，里面的信息都是可以运行的时候改变的。
  * 通过TaskContext.get()方法在任意地方获取Task的上下文，
  * 里面提供的很多信息都是运行状态的信息
 * {{{
...
abstract class TaskContext extends Serializable {
   ...
   * to an already completed task will result in that listener being called immediately.
    * 给Task任务加入监听器，完成，失败，取消等会触发监听器事件
   *
    * 一个例子是：给HadoopRDD注册一个callback，关闭输入流
   ...
   * This will be called in all situations - success, failure, or cancellation. Adding a listener
   * to an already completed task will result in that listener being called immediately.
    * 另一种初始化方式，通过传递函数，然后构造TaskCompletionListern
   *
   * An example use is for HadoopRDD to register a callback to close the input stream.
   *
   * Exceptions thrown by the listener will result in failure of the task.
   ...
   * The ID of the stage that this task belong to.
    * 每个Task总是属于某个stage的，这个非常重要！！！理解这些属于概念
   */
  def stageId(): Int
   ...
   * numbers.
    * 该Task所属的stage尝试执行的次数，第一次执行为0
   */
  def stageAttemptNumber(): Int

  /**
   * The ID of the RDD partition that is computed by this task.
    * 该task所执行的RDD partition的ID！！！这个非常重要，看来每个task执行一次执行一个partition
   */
  def partitionId(): Int
   ...
   * attemptNumber = 0, and subsequent attempts will have increasing attempt numbers.
    * Stage有很多task，这里是说该task尝试的执行的次数
   */
   ...
   * will share the same attempt ID).  This is roughly equivalent to Hadoop's TaskAttemptID.
    * 本次task的attemptId，注意！！在SparkContext里，所有的Task attempt，没有重复的
   */
   ...
   * `org.apache.spark.SparkContext.setLocalProperty`.
    * 在Driver中也可以指定task的localProperty！！通过配置文件
   */
  def getLocalProperty(key: String): String

  // 看来任务上下文其实是包含任务的指标！！这是应该的！！这样才能更好的监控任务
  @DeveloperApi
   ...
   * which runs the task. For more information see `org.apache.spark.metrics.MetricsSystem`.
    * 这个metrics sources概念要好好理解，这里解释是：所有运行该任务的所有对象！！我的理解是只要在该任务中运行的实例，就应该
    * 纳入指标要统计的范围！！
    *
    * Source是啥？scala定义的是一个source名字和一个MetricRegistry,
    * 为啥叫源，因为所有的Metrics的综合统计，就是从MetricRegistry这里来的，这里面提供了丰富的监控指标
   */
  @DeveloperApi
