...
private[memory] class StorageMemoryPool(
    lock: Object,      // 用于同步的锁
    memoryMode: MemoryMode   // 这个池子的内存类型 on-heap 还是 off-heap
  ) extends MemoryPool(lock) with Logging { // 继承了memorypool，可以看到就算继承也可以部分初始化的！！MemoryPool( lock ) !!!
  ...
  // 使用了多少memory，GuardedBy表示只有拥有了lock这个锁才能够访问这个域
  @GuardedBy("lock")
