 ...
 * components, including automatic selection of which [[Serializer]] to use for shuffles.
  * 集中用于管理 序列化、压缩、编码  的组件的管理器
 */
private[spark] class SerializerManager(
    defaultSerializer: Serializer,       // 序列化管理器当然要有序列化器的字段了
    conf: SparkConf,                     // 配置文件
    encryptionKey: Option[Array[Byte]]) {  // 压缩键？？

  // 默认没有压缩
  def this(defaultSerializer: Serializer, conf: SparkConf) = this(defaultSerializer, conf, None)

  // kryo序列化器,为啥单独弄一个kryo的序列化器？
  // kryo序列化器是谷歌的一个库的封装
  private[this] val kryoSerializer = new KryoSerializer(conf)

  // java的ClassLoader类，里面定义若干加载类的方法
  ...
  // Shuffle的输出需要压缩么。  注意！！ 一定是Shuffle的  输出！！
   ...
   * loaded yet. */
  // 加载压缩编码器，注意使用的是lazy。。第一次针对需要使用这个再加载，因为后面用户可能还会加载第三方压缩jar
   ...
   * Pick the best serializer for shuffling an RDD of key-value pairs.
    * 对于元素为k-v对的RDD，可以看到如果判断k和v都适合使用kryo的话，那么还是优先使用kryo的！！
   */
  ...
  // 判断BlockId是不是可以压缩？？首先判断BlockId的类型，然后根据特定的类型，查找配置决定是不是压缩
  ...
  def dataSerializeStream[T: ClassTag](
    ...
    val autoPick = !blockId.isInstanceOf[StreamBlockId] // 继承自blockid，通过streamid和uniqueid唯一标识StreamBlockid
