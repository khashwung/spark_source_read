...
import java.util.concurrent.ConcurrentHashMap   // java多线程类
...
/**
 * Configuration for a Spark application. Used to set various Spark parameters as key-value pairs. 键值对配置
 *
 * Most of the time, you would create a SparkConf object with `new SparkConf()`, which will load
 * values from any `spark.*` Java system properties set in your application as well. In this case,
 * parameters you set directly on the `SparkConf` object take priority over system properties.
  *  可以通过java system properties设置，也可以通过SparkConf对象设置，优先级： system properties < SparkConf
  *  java系统设置是通过spark.*的配置设置的
 *
 ...
 * `new SparkConf().setMaster("local").setAppName("My app")`.
  *  支持chaining语法！！这应该是Builder模式！！
 *
 * @param loadDefaults whether to also load values from Java system properties
 *
 * @note Once a SparkConf object is passed to Spark, it is cloned and can no longer be modified
 * by the user. Spark does not support modifying the configuration at runtime.
  * SparkConf不支持运行时修改，用SparkConf初始化Spark运行时后就不能改变了
 */
class SparkConf(loadDefaults: Boolean) extends Cloneable with Logging with Serializable {
  ...
  /** 默认从系统和类路径加载配置 system properties and classpath*/
  def this() = this(true)

  // 存储所有的键值对设置的是一个跨线程的Map，这应该是这个类最核心的存储结构了！！
  ...
  private[spark] def set(key: String, value: String, silent: Boolean): SparkConf = {
    // 若干null判断，看来scala中也是有不少null判断的逻辑的,以及抛异常逻辑...
    if (key == null) {
  ...
  def setJars(jars: Seq[String]): SparkConf = {
    // 检验一遍是不是有null，如果有null，程序会忽略，但是还是要报个warning的
  ...
  // 这些k-v对中的key都是诸如spark.home，即以spark开头的
