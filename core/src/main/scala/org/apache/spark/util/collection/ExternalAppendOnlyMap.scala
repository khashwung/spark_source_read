 ...
 * to grow.
  * 这是一个只能够添加元素的Map，用来：当没有足够的内存时，存储要放进disk的容量
 *
 ...
 * non-spilling AppendOnlyMap.
  * 设置spill的阈值时存在一个权衡的：
  * 1.阈值太高，map在内存里占据了太多的空间，容易产生OOM；
  * 2.阈值太低，会很容易产生磁盘读写，损失性能；
 */
@DeveloperApi
class ExternalAppendOnlyMap[K, V, C](
    createCombiner: V => C,                      // 所谓combiner，就是一个函数：值V => Combiner C
    mergeValue: (C, V) => C,                     // 所谓mergeValue，就是将值V和Combiner C融入起来得到新的Combiner C
    mergeCombiners: (C, C) => C,                 // 所谓mergeCombiner，就是将两个Combiner C融合为一个Combiner C
    serializer: Serializer = SparkEnv.get.serializer,  // 序列化器，从SparkEnv中获得的，这里面封装了很多环境变量
