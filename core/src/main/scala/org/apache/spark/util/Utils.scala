...
private[spark] object Utils extends Logging {
   ...
   * Get the ClassLoader which loaded Spark.
    * 获取类加载器：首先获取类，后获取当前类的类加载器。这是跨线程的类加载器，而不是某个线程的类加载器！！
   */
   ...
   * loaded Spark.
    * 如果当前线程有ClassLoader，那么就获取；如果没有，就获取Spark的Classloader。注意！！
    * Context在程序中通常是当前线程的语义！！！，这是小技巧
   *
   * This should be used whenever passing a ClassLoader to Class.ForName or finding the currently
   * active loader when setting up ClassLoader delegation chains.
   */
  def getContextOrSparkClassLoader: ClassLoader =
    // 这里，java的原始Thread里含有getContextClassLoader方法，该方法检查当前线程是否具有访问能力，如果有就返回当前
    // Thread的ClassLoader
  ...
  def classForName(className: String): Class[_] = {
    // java中的加载类，是需要传递类加载器的，类加载器已经由Utils中的getcontextOrSparkClassLoader函数提供了
   ...
   * exception
    * 执行一个代码块，可以看到参数block是=>Unit类型，即传值，进函数后才执行
    * 如果出错就直接停掉SparkContext
   *
   * NOTE: This method is to be called by the driver-side components to avoid stopping the
   * user-started JVM process completely; in contrast, tryOrExit is to be called in the
   * spark-started JVM process .
    * 总共有两种process：user-started和spark-started
    * 这个是driver-side的组件调用的，避免完全关闭用户开启的jvm进程？？
   */
  def tryOrStopSparkContext(sc: SparkContext)(block: => Unit) {
    try {
      // 传值代码块不需要通过()方式执行，直接写一下，该代码块就执行了
      block
    } catch {
      case e: ControlThrowable => throw e
      case t: Throwable =>
  ...
  def getCallSite(skipClass: String => Boolean = sparkInternalExclusionFunction): CallSite = {
    ...
    // Thread.getStackTrace  获取当前线程对应的方法栈！
    // StackTraceElement对象中定义了诸多方法，可以获得
  ...
  */
  ...
  def getSystemProperties: Map[String, String] = {
    // System.getProperties常用的java方法，获取所有的系统变量 Property
    // Property.stringPropertyNames这个方法返回所有的keyset
    // asScala貌似非常有用！！从java到Scala集合的转换
    System.getProperties.stringPropertyNames().asScala
      .map(key => (key, System.getProperty(key))).toMap
    // 所以总结一下，该方法就是完成了java获取的系统变量到scala系统变量的包装
  }
  ...
  // 注意，传名引用的f进函数后，只需要f，（和函数类型传入需要f()才能求值不同），就可以获得其值！！并且f如果是个方法体，里面的计算都是在传入后计算的
  def tryLog[T](f: => T): Try[T] = {
    try {
      // Try包含两个子类：Success和Failure，和Option,Some,None之间的关系一样，作为Wrapper来用
      // 下面的f就已经算出了一个T类型的结果了
      val res = f
