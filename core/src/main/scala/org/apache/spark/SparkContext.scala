 ...
 * cluster, and can be used to create RDDs, accumulators and broadcast variables on that cluster.
  * 使用Spark函数的入口点！！ 一个SparkContext代表一个到集群的连接
 *
 * Only one SparkContext may be active per JVM.  You must `stop()` the active SparkContext before
 * creating a new one.  This limitation may eventually be removed; see SPARK-2243 for more details.
  * 一个JVM只允许一个SparkContext!!，日后可能不会有这种限制
 *
...
class SparkContext(config: SparkConf) extends Logging {
  ...
  // SparkConf里包含了几乎所有重要的组件！！以私有变量存在这里
   ...
   * changed at runtime.
    * 以为配置不能在运行时被改变，所以这里返回了一个安全的clone版本
   */
  ...
  try {
    ...
    // 这两个必须得设置:spark.master,spark.app.name
    ...
    // 判断yarn和cluster的部署模式下，一定也要设置xxx，要不然报错
    ...
    // 获取所有的jar包，其实就是从SparkConf中的spark.jars记录里获取
    ...
    // eventLog压缩编码器的配置，如果需要配置返回编码器的名字，注意是名字，而不是压缩编码器类。如果不需要为None
    _eventLogCodec = {
   ...
   * SparkContext instead.
    * 在SparkContext内部停止服务，容易造成死锁（互相要对方的锁，才能解开对方要自己的锁），用这种方法不会（单独开辟守护线程），为啥？
   */
  private[spark] def stopInNewThread(): Unit = {
    new Thread("stop-spark-context") {
      // 设置守护线程，注意，不是守护进程，守护线程随jvm死（只剩下守护线程了）而死
      setDaemon(true)
