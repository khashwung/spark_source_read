 ...
 * set of keys which produce its unique name.
  * blockid 用于唯一标识一个block数据，通常这个block数据都是一个文件，
  * BlockId内部采用一个唯一的文件命名filename来标识
 *
 * If your BlockId should be serializable, be sure to add it to the BlockId.apply() method.
 */
// 这是一个抽象基类，具体可以分为很多种不同的实现类，根据这个blockId所表示的东西的不同
@DeveloperApi
sealed abstract class BlockId {
  /** A globally unique identifier for this Block. Can be used for ser/de. */
  // 实际上通过唯一的name来进行标识一个blockId
  def name: String
...
// RDD的block唯一标识，rdd + rdd的id + split索引
@DeveloperApi
case class RDDBlockId(rddId: Int, splitIndex: Int) extends BlockId {
  override def name: String = "rdd_" + rddId + "_" + splitIndex
}

// RDD好理解，shuffle是一种操作也是block么？
...
object BlockId {
  ...
  // 统一通过正则表达式进行模式匹配，然后初始化这个字符串所代表的BlockId类别
