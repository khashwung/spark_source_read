...
private[storage] trait BlockEvictionHandler {
   ...
   * store reaches its limit and needs to free up space.
    * 当内存达到limit，需要清理时，就用该方法清理内存中的block数据，很可能是写入磁盘
   *
   * If `data` is not put on disk, it won't be created.
   *
   * The caller of this method must hold a write lock on the block before calling this method.
   * This method does not release the write lock.
    * 调用该方法的时候一定要拿到该block的写锁！！！这段时间不允许别人写的
   *
 ...
 * serialized ByteBuffers.
  * 存储memory的blocks。可以存为: Arrays of deserialized Java objects
  * 或者： serialized ByteBuffers
 */
private[spark] class MemoryStore(
    conf: SparkConf,
    blockInfoManager: BlockInfoManager,  //
    serializerManager: SerializerManager,  // 序列化器管理器
    memoryManager: MemoryManager,          // 内存管理器
    blockEvictionHandler: BlockEvictionHandler)  // 上面定义的handler
  extends Logging {
