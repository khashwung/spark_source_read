 ...
 * retrieving blocks both locally and remotely into various stores (memory, disk, and off-heap).
  * 所有的节点（driver还是executor）都会有这样的BlockManager，提供获取blocks的能力，（不管来自于locally还是remotely，
  * 是memory，disk还是off-heap 存储）
 *
 * Note that [[initialize()]] must be called before the BlockManager is usable.
 */
private[spark] class BlockManager(
    executorId: String,  // 因为每个节点有一个BlockManager，所以每个BlockManager和一个executor唯一关联，需要存个这个
    rpcEnv: RpcEnv,      // 管理Rpc调用环境的，该Rpc环境注册了几个可以调用的RpcEndpoint
    val master: BlockManagerMaster, // 用于和driver通讯的一个BlockManger,可以获得整个集群的很多信息
    val serializerManager: SerializerManager,  // 管理各种压缩,序列化以及编码方法
    val conf: SparkConf,
