 ...
 * @param classTag the block's [[ClassTag]], used to select the serializer
  *                 根据这个block的ClassTag，可以选择序列化器？？
 * @param tellMaster whether state changes for this block should be reported to the master. This
 *                   is true for most blocks, but is false for broadcast blocks.
  *                   是否改变需要通知Master？大部分Block都需要，然而Broadcast Block不需要！！
 */
private[storage] class BlockInfo(
   ...
   */
  // writeTask是存储一个task attempt id，然后这个id所对应的task是存储着对于这个block的写锁
  def writerTask: Long = _writerTask
  def writerTask_=(t: Long): Unit = {
    _writerTask = t
    checkInvariants()
  }
  // 如果没有写锁锁着这个block，那么就是NO_WRITER
  //
...
private[storage] object BlockInfo {
   ...
   * a non-task thread (e.g. by a driver thread or by unit test code).
    * 由非task相关的线程锁保持的写锁，非task线程指的比如driver线程或者单元测试线程
   */
 ...
 * Component of the [[BlockManager]] which tracks metadata for blocks and manages block locking.
  * 所有的BlockInfo也需要一个管理者BlockInfoManager！！
  * 这个用来管理Block的元数据以及锁
 *
...
private[storage] class BlockInfoManager extends Logging {
   ...
   * by [[removeBlock()]].
    *
   */
    // 只有获取这个BlockInfoManager的实例才能够访问所有的BlockInfo信息
  @GuardedBy("this")
   ...
   */
    // 查看了MultiMap的scaladoc，发现其实下面这种使用方法挺常见的，MultiMap通常用于mixin，用于一个map的key有多个value
    // 一个task对应多个BlockId,这就是记录这个信息的集合
  @GuardedBy("this")
  private[this] val writeLocksByTask =
    new mutable.HashMap[TaskAttemptId, mutable.Set[BlockId]]
      with mutable.MultiMap[TaskAttemptId, BlockId]    // 直接在初始化这里用with初始化！！这个就可以混入特质了
