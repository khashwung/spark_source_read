 ...
 * through implicit.
  * 这段总结以下几点：
  * 1.RDD是编程抽象，特点是：不可变，可分割，可并行处理
  * 2.这篇文件定义的是RDD抽象基类，所以里面的方法适用于所有的RDd
  * 3.  PairRDDFunctions          包含  操作  on   RDD of k-v pair
  *     DoubleRDDFunctions        包含  操作  on   RDD of Doubles
  *     SequenceFileRDDFunctions  包含  操作  on   RDD that can be saved as SequenceFiles
  *     注意这三个都是RDDFunctions，而不是RDD
  *
  *     这些RDDFunction如何对特定的RDD起作用呢？？当然是scala独具特色的implicit啦
  *
 *
 ...
 *    an HDFS file)
  *
  * RDD抽象到底包含哪些重要的属性？包括以下5个
  * 1. 包含的partitions， list对象
  * 2. 一个函数对象，每个split，或者叫partition都采用这个函数对象计算，所以大数据的核心是移动计算而不是移动数据！！体现在这里
  * 3. 对其它RDD的依赖，是一个list对象。这个依赖还是蛮重要的概念
  * 4. 一个Partitioner，可选的，如何partition 我们RDD的k-v对？就由这个Partitioner决定
  * 5. 若干个较为偏向的location地址，啥意思？有啥用？为啥存这个？？还是那句话，Spark是偏向于移动计算而不是数据，下发计算函数的时候当然是往性能更好的block那里移动计算啦
 *
 ...
 * for more details on RDD internals.
  * 用户是可以customize自己的RDD的，针对不同的存储系统！！
 */
// ClassTag，将编译器推导的类型传到运行时使用！！
abstract class RDD[T: ClassTag](
    @transient private var _sc: SparkContext,
    @transient private var deps: Seq[Dependency[_]]
  ) extends Serializable with Logging {
  // 上下文和一系列依赖。 并且这两个不用序列化

  // java的一个本地方法，class A isAssignableFrom class B，用来测试B是否是A或者是A的子类？
  // ClassTag.runtimeClass 用于获得运行时真正的T类型
  ...
  // 根据这个变量名可以看出，这是表示RDD里的元素类型
  // classTag是scala reflect对象里的方法，如果隐式作用域中有ClassTag[T]类，那么就把这个找出来返回
